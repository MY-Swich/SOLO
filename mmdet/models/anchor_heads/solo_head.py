...
def dice_loss(input, target):
    ...
    a = torch.sum(input * target, 1)   # input与target先点乘， 然后依照行元素相加
    b = torch.sum(input * input, 1) + 0.001
    c = torch.sum(target * target, 1) + 0.001  #为啥要加上0.01没搞懂 可能是为了防止分母为0
    d = (2 * a) / (b + c) # Dice系数
    return 1-d  # dice loss
...
class SOLOHead(nn.Module):
    ...
    def __init__(self,
        ...
        self.seg_num_grids = num_grids  #网格数量
        self.cate_out_channels = self.num_classes - 1 #通道
